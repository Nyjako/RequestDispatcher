 ...
 * @brief Manages and dispatches HTTP requests using libcurl while enforcing a
...
/**
 * @brief The RequestDispatcher class manages and dispatches HTTP requests using
 * libcurl while enforcing a specified rate limit.
 *
 */
class REQUEST_DISPATCHER_EXPORT RequestDispatcher
{
  public:
     ...
     * @return std::future<CURLcode> that the user can use to wait for and
  ...
  private:
    ...
    /**
     * @brief The duration (in seconds) of the time frame within which requests
     * are rate-limited.
     *
     */
    ...
    /**
     * @brief The maximum number of requests that can be sent within the
    ...
    /**
     * @brief A flag indicating whether the worker thread should stop processing
     * requests.
     *
     */
    bool m_stop_worker;
    ...
    /**
     * @brief A queue that holds pairs of CURL* handles and their corresponding
     * std::promise<CURLcode> objects.
     *
     */
    std::queue<std::pair<CURL*, std::promise<CURLcode>>> m_request_queue;

    /**
     * @brief A thread that processes the request queue and sends requests
     * asynchronously.
     *
     */
